    ...
    ({ instance, size: initialSize, zoom: initalZoom }) => {
        ...
        useLayoutEffect(() => {
            ...
            const onWheel = async (e: WheelEvent) => {
                if (instance.state.mouseOver && instance.state.mouseDown && !isShiftDown.current) {
                    const val =
                        zoom.current +
                        (e.deltaY / 100) * (settings.store.invertScroll ? -1 : 1) * settings.store.zoomSpeed;
                ...
                if (instance.state.mouseOver && instance.state.mouseDown && isShiftDown.current) {
                    const val =
                        size.current + e.deltaY * (settings.store.invertScroll ? -1 : 1) * settings.store.zoomSpeed;
        ...
        const imageSrc = useMemo(() => {
            try {
                ...
                if (imageUrl.pathname.startsWith("/attachments/")) imageUrl.hostname = "cdn.discordapp.com";
