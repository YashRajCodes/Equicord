...
const settings = definePluginSettings(
    ...
    },
    {
        randomisedLength: {
            disabled() {
                return this.store.method !== Methods.Random;
            }
        },
        consistent: {
            disabled() {
                return this.store.method !== Methods.Consistent;
            }
        },
        dateFormat: {
            disabled() {
                return this.store.method !== Methods.Date;
            }
        }
    }
);
    ...
    AnonymiseUploadButton: ErrorBoundary.wrap(
        ({ upload }: { upload: CloudUpload }) => {
            const [anonymise, setAnonymise] = useState(
                upload[ANONYMISE_UPLOAD_SYMBOL] ?? settings.store.anonymiseByDefault
            );
