...
export const Checkbox = waitForComponent<t.Checkbox>(
    "Checkbox",
    filters.componentByCode('"data-toggleable-component":"checkbox')
);

export const Tooltip = waitForComponent<t.Tooltip>(
    "Tooltip",
    m => m.prototype?.shouldShowTooltip && m.prototype.render,
    TooltipFallback
);
...
export const TextInput = waitForComponent<t.TextInput>(
    "TextInput",
    filters.componentByCode('setHasValue?.(""!==', '="text",')
);
export const TextArea = waitForComponent<t.TextArea>(
    "TextArea",
    filters.componentByCode("!0,rows:", "showRemainingCharacterCount:")
);
export const Select = waitForComponent<t.Select>(
    "Select",
    filters.componentByCode('selectionMode:"single",onSelectionChange:', "isSelected:")
);
export const SearchableSelect = waitForComponent<t.SearchableSelect>(
    "SearchableSelect",
    filters.componentByCode('?"multiple":"single",required:')
);
...
export const UserSummaryItem = waitForComponent(
    "UserSummaryItem",
    filters.componentByCode("defaultRenderUser", "showDefaultAvatarsForNullUsers")
);

export let createScroller:
    | ((scrollbarClassName: string, fadeClassName: string, customThemeClassName: string) => t.ScrollerThin)
    | undefined;
...
export const ScrollerNone = LazyComponent(() =>
    createScroller?.(scrollerClasses.none, scrollerClasses.fade, scrollerClasses.customTheme)!
);
export const ScrollerThin = LazyComponent(() =>
    createScroller?.(scrollerClasses.thin, scrollerClasses.fade, scrollerClasses.customTheme)!
);
export const ScrollerAuto = LazyComponent(() =>
    createScroller?.(scrollerClasses.auto, scrollerClasses.fade, scrollerClasses.customTheme)!
);

export const ListScrollerThin = LazyComponent(() =>
    createListScroller(listScrollerClasses.thin, listScrollerClasses.fade, "", ResizeObserver)
);
export const ListScrollerAuto = LazyComponent(() =>
    createListScroller(listScrollerClasses.auto, listScrollerClasses.fade, "", ResizeObserver)
);
...
waitFor(
    m => {
        ...
        const str = String(m);
        return (
            str.includes(".resolve({theme:") &&
            str.includes('"refresh-fast-follow-avatars"') &&
            !str.includes("useMemo")
        );
    },
    m => (useToken = m)
);
...
export const OAuth2AuthorizeModal = waitForComponent(
    ...
    filters.componentByCode("hasContentBackground", "nextStep", "onClose?.()")
);
