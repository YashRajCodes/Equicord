        ...
        <>
            ...
            {activities.length === 0 ? (
                ...
                activities.map(activity => {
                    const isSpotify =
                        (activity.flags & (ActivityFlags.SYNC | ActivityFlags.PLAY)) ===
                        (ActivityFlags.SYNC | ActivityFlags.PLAY);
                    return !isSpotify ? (
                        <Paragraph>
                            {activity.name}: {activity.application_id}
                        </Paragraph>
                    ) : null;
                })
            )}
            ...
            <Paragraph>
                In all fields (except stream URL), you can put in variables that'll automatically be replaced by their
                original content:
...
export function ReplaceSettings({ appIds, update, save }: SettingsProps) {
    async function onChange(val: string | boolean, index: number, key: string) {
        if (index === appIds.length - 1) appIds.push(makeEmptyAppId());
                    ...
                    {isValidSnowflake(setting.appId) && (
                        <>
                            ...
                            {setting.newActivityType === ActivityType.STREAMING && (
                                <>
                                    <Heading className={`${Margins.top8} ${Margins.bottom8}`}>
                                    ...
                                    <CheckedTextInput
                                        ...
                                        validate={v => {
                                            return (
                                                /https?:\/\/(www\.)?(twitch\.tv|youtube\.com)\/\w+/.test(v) ||
                                                "Invalid stream URL"
                                            );
                                        }}
                                    />
                            ...
                            {setting.newActivityType !== ActivityType.STREAMING && (
                                <>
                                    <Heading className={Margins.top8}>
                                        New name {setting.newActivityType === ActivityType.PLAYING && "(first line)"}
                                    </Heading>
                                    <TextInput
                                        className={Margins.top8}
                            ...
                            <Heading className={Margins.top8}>
                                New details{" "}
                                {setting.newActivityType === ActivityType.PLAYING ? "(second line)" : "(first line)"}
                            </Heading>
                            <TextInput
                                className={Margins.top8}
                            ...
                            <Heading className={Margins.top8}>
                                New state{" "}
                                {setting.newActivityType === ActivityType.PLAYING ? "(third line)" : "(second line)"}
                            </Heading>
                            <TextInput
                                className={Margins.top8}
