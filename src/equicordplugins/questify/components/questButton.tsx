...
function QuestButtonView({
    ...
    const icon = <div className={iconContainerClass}>{QuestIcon(26, 26)}</div>;
...
export function showQuestButton(
    displayMode: QuestButtonDisplayMode,
    badgeCount: number,
    isOnQuestsPage: boolean
): boolean {
...
export function disguiseHomeButton(pathname: string): boolean {
    ...
    if (
        !showQuestButton(
            buttonSettings.questButtonDisplay,
            buttonSettings.questButtonBadgeCount,
            buttonSettings.isOnQuestsPage
        )
    ) {
        return false;
    }
...
function getQuestButtonLowerBadgeProps(
    ...
    return {
        ...
        maxDigits: 2,
        ...(resolvedColor ? { color: `rgb(${resolvedColor.r}, ${resolvedColor.g}, ${resolvedColor.b})` } : {}),
        ...(resolvedColor ? { style: { color: isDarkish(resolvedColor) ? "white" : "black" } } : {})
    };
}
...
export function QuestButtonContextMenu({ dummy = false }: { dummy?: boolean }): JSX.Element {
    ...
    return (
        <Menu.Menu navId={navId} onClose={ContextMenuApi.closeContextMenu} aria-label="Quest Button Menu">
            <Menu.MenuItem
            ...
            <Menu.MenuItem
                ...
                label="Fetch Quests"
                action={() => {
                    void fetchAndAlertQuests("MANUAL_FETCH");
                }}
