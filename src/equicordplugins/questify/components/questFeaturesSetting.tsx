...
import {
    autoCompleteQuestTaskTypes,
    defaultAllowChangingDangerousSettings,
    defaultAutoCompleteQuestsSimultaneously,
    defaultAutoCompleteQuestTypes,
    defaultCompleteVideoQuestsQuicker,
    defaultMakeMobileVideoQuestsDesktopCompatible,
    defaultResumeInterruptedQuests,
    isDesktopCompatible
} from "../settings/def";
...
function SettingsAllowDangerousButton({ allowed, disabled, onClick }: SettingsAllowDangerousButtonProps): JSX.Element {
    return (
...
export function QuestFeaturesSetting(): JSX.Element {
    ...
    const selectedDisableValues = disableFeatureOptions.filter(({ key }) => questFeatures[key]).map(({ key }) => key);
    ...
    function updateAutoCompleteQuestTypes(value: string | string[] | null) {
        ...
        for (const { value: questType } of autoCompleteQuestTypeOptions) {
            const nextValue = selectedValues.has(String(questType)) && isDesktopCompatible(questType);
        ...
        <SettingsCard>
            ...
            <SettingsSelect
                ...
                tooltip={{
                    position: "top",
                    text:
                        "Sponsored Banner is a paid-for Quest banner at the top of the Quests page." +
            ...
            <SettingsNotice
                className={[
                    "notice-card-red",
                    questFeatures.disableQuestsEverything ? "dimmed-settings-item" : undefined,
                    questFeatures.allowChangingDangerousSettings ? undefined : "notice-card-solo",
                    "no-bottom-margin"
                ].filter(c => c !== undefined)}
            >
                <SettingsParagraph>
                    Discord has began issuing warnings to users of scripts or plugins that modify how Quests are
                    completed, which is against their{" "}
                    <a
                ...
                <br />
                <SettingsParagraph>
                    The warnings appear limited to threat of loss of access to Quests or their rewards, but Discord may
                    escalate at any time.
                </SettingsParagraph>
                <br />
                <SettingsParagraph>
                    Due to the various methods Discord uses to track users, there's no way to realistically evade
                    detection. If you proceed, understand that Discord likely will detect it at some point.
                </SettingsParagraph>
                <br />
                <SettingsParagraph>
                ...
                {questFeatures.allowChangingDangerousSettings && (
                    <>
                        <SettingsSubtleSwitch
                            disabled={
                                questFeatures.disableQuestsEverything || !questFeatures.allowChangingDangerousSettings
                            }
                            ...
                            tooltip={{
                                position: "top",
                                text:
                                    "Discord allows Video Quests to be completed once 24 seconds less than the duration of the video has passed since you enrolled into the Quest." +
                        ...
                        <SettingsSubtleSwitch
                            ...
                            tooltip={{
                                position: "top",
                                text:
                                    "Some mobile-only Video Quests can be enrolled in on desktop, but still must be completed on mobile. This setting will allow those to be completed on desktop." +
                        ...
                        <SettingsSubtleSwitch
                            ...
                            tooltip={{
                                position: "top",
                                text:
                                    "By default, attempting to auto-complete multiple Quests will queue them to be completed in order." +
                        ...
                        <SettingsSelect
                            ...
                            tooltip={{
                                ...
                                wider: true,
                                text:
                                    "Watch Video on Mobile Quests will only work on mobile Quests which are enrollable on desktop. If a Quest is locked to enrollment on mobile, you must first enroll in it on your mobile device before returning to desktop and refreshing your Quests." +
