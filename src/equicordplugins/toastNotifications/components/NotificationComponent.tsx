...
function renderContextHeader(channel: MessageNotification["channel"]): React.ReactNode {
    ...
    if (channel.guild_id) {
        ...
        const icon = guild?.icon ? IconUtils.getGuildIconURL({ id: guild.id, icon: guild.icon, size: 32 }) : undefined;
        return (
            <div className={cl("context-header")}>
                {icon && <img className={cl("context-header-icon")} src={icon} alt="" />}
...
export default ErrorBoundary.wrap(
    function NotificationComponent(props: NotificationData) {
            ...
            () => (
                ...
                >
                    <svg
                        ...
                        viewBox="0 0 24 24"
                        role="img"
                        aria-labelledby="vc-toast-notifications-dismiss-title"
                    >
