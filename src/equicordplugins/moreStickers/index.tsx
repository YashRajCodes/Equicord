...
export default definePlugin({
            ...
            replacement: [
                {
                    ...
                    replace:
                        ',vcStickers=$1?$2($3,{id:"stickers+-picker-tab","aria-controls":"more-stickers-picker-tab-panel","aria-selected":$4==="stickers+",isActive:$4==="stickers+",autoFocus:true,viewType:"stickers+",children:$5+"+"})}):null'
                },
                {
    ...
    stickerButton({ innerClassName, isActive, onClick }) {
        return (
            <button className={innerClassName} onClick={onClick} style={{ backgroundColor: "transparent" }}>
    ...
    moreStickersComponent({ channel, closePopout }: { channel: Channel; closePopout: () => void }) {
        ...
        React.useEffect(() => {
            (async () => {
                ...
                const sps = (await Promise.all(stickerPackMetas.map(meta => getStickerPack(meta.id)))).filter(
        ...
        return (
            <Wrapper>
                ...
                <PickerSidebar
                    packMetas={stickerPackMetas.map(meta => ({
                        id: meta.id,
                        name: meta.title,
