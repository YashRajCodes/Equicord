...
export function CategoryImage({ src, alt, isActive }: CategoryImageProps) {
    return (
        <div>
            ...
            >
                <foreignObject
                    className={cl("foreign-object") + (isActive ? ` ${cl("foreign-object-active")}` : "")}
...
export function CategoryWrapper(props: { children: JSX.Element | JSX.Element[] }) {
    return <div className={cl("category-wrapper")}>{props.children}</div>;
}
...
export function StickerCategory(props: StickerCategoryProps) {
    return (
        <div
            style={props.style}
            className={cl("sticker-category") + (props.isActive ? ` ${cl("sticker-category-active")}` : "")}
            tabIndex={0}
            role="button"
