...
export function SettingsPresetList(isResolution: boolean) {
    ...
    const errorString = useMemo(() => {
        ...
        if (isResolution) {
            if (value < MIN_RESOLUTION) return `Must be >${MIN_RESOLUTION}`;
            return;
        }
        if (value < MIN_FPS) return `Must be >${MIN_FPS}`;
    }, [value]);
                ...
                {(isResolution ? resolutions : fpss).map(({ label, value }, index) => (
                    <Card key={index} className={cl("settings-card")}>
                        <Flex gap={"0.5em"} alignItems="center" flexWrap="wrap">
                            <Paragraph size="sm" weight="semibold" className={cl("settings-preset-label")}>
                                {label}
                            </Paragraph>
                            <Button
                                ...
                                onClick={() => (isResolution ? resolutions : fpss).splice(index, 1)}
                            >
                ...
                <Card className={cl("settings-card")}>
                    <Flex gap={"0.5em"} alignItems="center" className={cl("settings-flex-input")}>
                        <TextInput
                            ...
                            error={errorString}
                        />
                        <Button size="iconOnly" disabled={!value || !!errorString} onClick={handleAddPreset}>
                            <PlusIcon />
                        </Button>
                    </Flex>
                </Card>
