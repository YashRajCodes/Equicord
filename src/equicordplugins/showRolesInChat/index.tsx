...
function toggleRole(roleId: string) {
    ...
    settings.store.excludedRoles = excluded.includes(roleId)
...
const HighestRoleIndicator = ErrorBoundary.wrap(
    ({ user, channelId }: { user: User; channelId: string }) => {
        ...
        const handleContextMenu = (e: React.MouseEvent) => {
            ContextMenuApi.openContextMenu(e, () => {
                ...
                const hiddenRoles = GuildRoleStore.getSortedRoles(guildId).filter(
                    r => excluded.includes(r.id) && userRoles.includes(r.id)
                );

                return (
                    <Menu.Menu
                        navId="vc-sric-context"
                        onClose={ContextMenuApi.closeContextMenu}
                        aria-label="Chat Role Actions"
                    >
                        <Menu.MenuGroup>
        ...
        return (
            <span className={cl("indicator")} onContextMenu={handleContextMenu}>
                ...
                <span className={cl("name")}>{role.name}</span>
            </span>
        );
    },
    { noop: true }
...
export default definePlugin({
    ...
    description:
        "Shows a user's highest role next to their name in chat messages. Hide/show specific roles in their context menu (right-click).",
    ...
    contextMenus: {
        "dev-context"(children, { id }: { id: string }) {
            if (GuildRoleStore.getRole(getCurrentGuild()?.id ?? "", id)) {
                children.push(<Menu.MenuGroup>{makeToggleRoleItem(id)}</Menu.MenuGroup>);
            }
        },
        "guild-settings-role-context"(children, props: { role: { id: string } }) {
            children.push(<Menu.MenuGroup>{makeToggleRoleItem(props.role.id)}</Menu.MenuGroup>);
        }
    },
    renderMessageDecoration: ({ message }) => (
        <HighestRoleIndicator user={message.author} channelId={message.channel_id} />
    )
});
