...
import {
    ChannelTabsProps,
    closeTab,
    ensureUnreadFallbackCountsLoaded,
    getNotificationDotState,
    getUnreadFallbackCounts,
    isTabSelected,
    moveDraggedTabs,
    moveToTab,
    openedTabs,
    settings,
    updateUnreadFallbackCounts
...
const GuildIcon = ({ guild }: { guild: Guild }) => {
    return guild.icon ? (
        ...
        <div className={cl("guild-acronym-icon")}>
            <BaseText size="xs" weight="semibold" tag="span">
...
function TypingIndicator({ isTyping }: { isTyping: boolean }) {
    return isTyping ? <ThreeDots dotRadius={3} themed={true} className={cl("typing-indicator")} /> : null;
}

function getChannelUnreadState(channelId: string) {
    const channel = ChannelStore.getChannel(channelId);
    const newForumPostCount =
        channel?.guild_id && channel.isForumLikeChannel?.()
...
export const NotificationDot = ({ channelIds }: { channelIds: string[] }) => {
    ...
    const channelStates = useStateFromStores([ActiveJoinedThreadsStore, ReadStateStore], () =>
        ...
        <div
            ...
            ref={node =>
                node?.style.setProperty(
                    "background-color",
                    hasMention ? "var(--red-400)" : "var(--brand-500)",
                    "important"
                )
            }
        >
...
export const TabNumberBadge = ({ number, position, isSelected, isCompact, isHovered }: TabNumberBadgeProps) => {
    ...
    return (
        <div className={cl("tab-number-badge", `position-${position}`)} data-position={position}>
            {number}
        </div>
    );
};
...
function ChannelTabContent(
    ...
    if (channel && recipients?.length) {
        if (recipients.length === 1) {
            ...
            const username = noPomeloNames ? user.globalName || user.username : getUniqueUsername(user);
            ...
            return (
                <>
                    <Avatar
                        ...
                        isMobile={isMobile}
                    />
                    <BaseText className={cl("name-text")}>{username}</BaseText>
                    <NotificationDot channelIds={[channel.id]} />
...
export default function ChannelTab(props: ChannelTabsProps & { index: number }) {
    ...
    return (
        <div
            ...
            onAuxClick={e => {
                if (e.button === 1 /* middle click */) closeTab(id);
            }}
        ...
        >
            ...
            {!compact && settings.store.showResizeHandle && (
                <div className={cl("tab-resize-handle")} onMouseDown={handleResizeStart} />
            )}
        </div>
    );
}
