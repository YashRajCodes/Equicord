...
function HideToggleButton(props: { open: boolean | undefined; onClick: MouseEventHandler<HTMLButtonElement> }) {
    return (
        <ChatBarButton onClick={props.onClick} tooltip={props.open ? "Close" : "Open"}>
            <svg
...
function ButtonsInnerComponent({ buttons }: { buttons: ReactNode }) {
    const buttonItems = Array.isArray(buttons) ? buttons : buttons == null ? [] : [buttons];
    ...
    return (
        <div
            key="chat-bar-buttons-menu"
            id="chat-bar-buttons-menu"
            style={{
                display: "flex",
                flexWrap: "nowrap",
                overflowX: "auto",
