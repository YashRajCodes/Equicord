...
async function onMessage(data: string) {
    ...
    switch (state.number) {
        case GameState.Play: {
            activity.type =
                profile.banchoStatus.number === BanchoStatusEnum.Playing ? ActivityType.PLAYING : ActivityType.WATCHING;
            ...
            combo =
                play.hits[0] === 0 && play.hits.sliderBreaks === 0
            ...
            const playRank = await getAsset(
                `https://raw.githubusercontent.com/AutumnVN/gosu-rich-presence/main/grade/${play.rank.current.toLowerCase().replace("x", "ss")}.png`
            );
        ...
        case GameState.ResultScreen: {
            ...
            fc =
                resultsScreen.maxCombo === beatmap.stats.maxCombo
                    ? "FC"
                    : `| ${resultsScreen.maxCombo}x/${beatmap.stats.maxCombo}x`;
            ...
            const resultRank = await getAsset(
                `https://raw.githubusercontent.com/AutumnVN/gosu-rich-presence/main/grade/${resultsScreen.rank.toLowerCase().replace("x", "ss")}.png`
            );
