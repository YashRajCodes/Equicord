    ...
    return (
        <SettingsSection tag="label" inlineSetting id={name} name={name} description={description}>
            <Switch
                ...
                onChange={v => {
                    setValue(v);
                    (settings.store[settingsKey] as boolean) = v;
                }}
            />
        </SettingsSection>
    );
    ...
    return (
        <SettingsSection id={name} name={name} description={description}>
            <TextInput
                ...
                onChange={v => {
                    setValue(v);
                    (settings.store[settingsKey] as string) = v;
                }}
...
function AudioBookShelfSettings() {
    return (
        <>
            <SettingsSection
                id="audiobookshelf-settings"
                name=""
                description="Display your currently playing audiobooks as Discord Rich Presence. Requires your AudioBookShelf server URL, username, and password."
            />
            <TextSetting
                name="Server URL"
                description="AudioBookShelf server URL."
                settingsKey="abs_serverUrl"
...
function TosuSettings() {
    return (
        <SettingsSection
            id="tosu-settings"
            name=""
            description="Connects to tosu via WebSocket on port 24050. No configuration needed, just make sure tosu is running alongside osu!."
        />
    );
}
    ...
    return (
        <>
            <SettingsSection
                id="statsfm-settings"
                name=""
                description="Show what you're currently listening to via stats.fm. Requires your listening history to be public."
            />
            <TextSetting
                name="Username"
                description="Stats.fm username."
                settingsKey="sfm_username"
                placeholder="stats.fm username"
            />
            <TextSetting
                name="Custom Status Text"
                description="Custom status text."
                settingsKey="sfm_statusName"
                placeholder="Stats.fm"
            />
            <SelectSetting
                name="Name Format"
                description="Name format."
                settingsKey="sfm_nameFormat"
                options={nameFormatOptions}
            />
            <SelectSetting
                name="Missing Art Fallback"
                description="Fallback when art is missing."
                settingsKey="sfm_missingArt"
                options={[
            ...
            <SwitchSetting
                ...
                settingsKey="sfm_useListeningStatus"
            />
            <SwitchSetting
                name="Show Stats.fm Logo"
                description="Show Stats.fm logo next to album art."
                settingsKey="sfm_showLogo"
            />
            <SwitchSetting
                name="Show Profile Link"
                description="Show link to stats.fm profile."
                settingsKey="sfm_shareUsername"
            />
            <SwitchSetting
                name="Show Song Link"
                description="Show link to song on stats.fm."
                settingsKey="sfm_shareSong"
            />
            <SwitchSetting
                name="Hide With Spotify"
                description="Hide stats.fm presence if Spotify is running."
                settingsKey="sfm_hideWithSpotify"
            />
            <SwitchSetting
                name="Hide With External RPC"
                description="Hide stats.fm presence if an external RPC is running."
                settingsKey="sfm_hideWithExternalRPC"
            />
            <SwitchSetting
                name="Disable Album Art"
                description="Disable downloading album art."
                settingsKey="sfm_alwaysHideArt"
            />
        </>
    );
}
    ...
    return (
        <>
            ...
            <TextSetting
                ...
                settingsKey="jf_serverUrl"
                placeholder="https://jellyfin.example.com"
            />
            <TextSetting
                name="API Key"
                description="Jellyfin API key."
                settingsKey="jf_apiKey"
                placeholder="Authorization"
            />
            <TextSetting
                name="User ID"
                description="Jellyfin user ID."
                settingsKey="jf_userId"
                placeholder="User ID from profile URL"
            />
            <SelectSetting
                name="Name Display"
                description="Name display format."
                settingsKey="jf_nameDisplay"
                options={[
            ...
            />
            <TextSetting
                name="Custom Name Template"
                description="Custom name template."
                settingsKey="jf_customName"
                placeholder="{name} on Jellyfin"
            />
            <SelectSetting
                name="Cover Type"
                description="Cover type for TV shows."
                settingsKey="jf_coverType"
                options={[
            ...
            />
            <SelectSetting
                name="Override Presence Type"
                description="Override rich presence type."
                settingsKey="jf_overrideType"
                options={[
                    { label: "Off", value: "off" },
...
function GensokyoRadioSettings() {
    ...
    return (
        <>
            <SettingsSection
                id="gensokyoradio-settings"
                name=""
                description="Discord rich presence for Gensokyo Radio. Just enable it and listen!"
            />
            <SettingsSection id="refresh-interval" name="Refresh Interval" description="Refresh interval in seconds.">
                <Slider
                    ...
                    onValueChange={v => {
                        setValue(v);
                        settings.store.gr_refreshInterval = v;
                    }}
                    onValueRender={v => `${v}s`}
                    stickToMarkers
                />
...
function NavidromeSettings() {
    ...
    return (
        <>
            <SettingsSection
                id="navidrome-settings"
                name=""
                description="Show what you're currently listening to via Navidrome."
            />
            <TextSetting
                name="Server URL"
                description="Navidrome Server URL (must be a public domain, e.g. https://navidrome.example.com)."
                settingsKey="nd_serverUrl"
            ...
            <TextSetting
                ...
                settingsKey="nd_clientId"
                placeholder="1470554657506984069"
            />
            <SelectSetting
                name="Album Art Mode"
                description="How to fetch album art."
                settingsKey="nd_albumArtMode"
                options={[
            ...
            {nd_albumArtMode === "lastfm" && (
                <TextSetting
                    ...
                    description="Optional: Provide your own Last.fm API Key (leaves default if empty)."
                    settingsKey="nd_lastfmApiKey"
                    placeholder="feff915bf5987580c9dc354d523dc6b9"
                />
            )}
            <SwitchSetting
                name="Show Small Image"
                description="Show Navidrome logo in bottom right of album art."
                settingsKey="nd_showSmallImage"
            />
            <SwitchSetting name="Show Album" description="Show album name in presence." settingsKey="nd_showAlbum" />
            <SelectSetting
                name="Activity Type"
                description="Which type of activity to display."
                settingsKey="nd_activityType"
                options={[
                    { label: "Listening", value: 2 },
                    { label: "Playing", value: 0 },
                    { label: "Watching", value: 3 }
            ...
            />
            <SelectSetting
                name="Status Display Type"
                description="Show the state / details line contents in the member list"
                settingsKey="nd_statusDisplayType"
                options={[
            ...
            <SwitchSetting
                name="Hide On Pause"
                description="Hide Rich Presence when music is paused (instead of showing a stopwatch)"
                settingsKey="nd_hideOnPause"
            />
            {Number(nd_activityType ?? 2) !== 0 && (
                <TextSetting
                    name="Activity Name Format"
                    description="The 'Listening to' text (e.g. {artist})"
                    settingsKey="nd_nameString"
                    placeholder="Navidrome"
                />
            )}
            <TextSetting
                name="Details Format"
                description="The main line showing what song is playing (e.g. {song})"
                settingsKey="nd_detailsString"
                placeholder="{song}"
            />
            <TextSetting
                name="State Format"
                description="The line below the song name (e.g. {artist})"
                settingsKey="nd_stateString"
                placeholder="{artist}"
            />
            <TextSetting
                name="Large Text Format"
                description="The album line (e.g. {album})"
                settingsKey="nd_largeTextString"
                placeholder="{album}"
            />
            <SettingsSection
                id="nd-refresh-interval"
            ...
            >
                <Slider
                    ...
                    onValueChange={v => {
                        ...
                        settings.store.nd_refreshInterval = v;
                    }}
                    onValueRender={v => `${v}s`}
                    stickToMarkers
                />
    ...
    return (
        <div className={classes("vc-plugins-settings", Margins.top16)}>
            ...
            <SwitchSetting
                key={currentTab}
                name="Enabled"
                description={`Enable ${TAB_LABELS[currentTab]} presence.`}
                settingsKey={ENABLE_KEYS[currentTab]}
            />
            <TabComponent />
        </div>
    );
