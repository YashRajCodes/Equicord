...
(async () => {
    try {
        ...
        paths.forEach(path => {
            ...
            idbSavedImages.set(str.split("/")?.[1]?.split(".")?.[0], {
                attachmentId: str.split("/")?.[1]?.split(".")?.[0],
                path: str
            });
        });
    } catch (err) {
...
export async function getImage(attachmentId: string, fileExt?: string | null): Promise<any> {
    ...
    if (idbPath) return get(idbPath, ImageStore);
...
export async function deleteImage(attachmentId: string): Promise<void> {
    const idbPath = idbSavedImages.get(attachmentId)?.path;
    if (idbPath) return await del(idbPath, ImageStore);
