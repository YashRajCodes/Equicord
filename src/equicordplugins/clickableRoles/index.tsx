...
function RoleMembersList({
    ...
    guildId: string;
    closePopout(): void;
    setPopoutRef(ref: HTMLDivElement | null): void;
}) {
    ...
    const users = useStateFromStores([UserStore], () => memberIds.map(id => UserStore.getUser(id)).filter(isTruthy), [
        memberIds
    ]);
    ...
    return (
        <div className={cl("popout")} ref={setPopoutRef}>
            <div className={cl("header")}>
                <span
                    className={cl("color")}
                    style={{ backgroundColor: role?.colorString ?? "var(--background-mod-strong)" }}
                />
