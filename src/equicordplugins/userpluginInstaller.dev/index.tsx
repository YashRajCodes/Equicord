...
export const settings = definePluginSettings({
    allowlistedChannels: {
        ...
        description:
            "Comma separated list of channels where the Install Plugin button should be displayed. It is always displayed in the Vencord Userplugin channels"
    },
    notifyIfUpdate: {
    ...
    neverNotifyForPlugins: {
        type: OptionType.STRING,
        description:
            "Never show update notifications for these plugins (you can still update them from the UserPlugins tab)",
        default: ""
    },
    setGitPath: {
...
export default definePlugin({
    ...
    settingsAboutComponent: () => (
        <Notice.Warning>
            Equicord does not moderate userplugins and takes no responsibility for anything that may result from
            installing them. Only install userplugins from developers you trust. Doing so is entirely at your own risk.
        </Notice.Warning>
    ),
    ...
    async start() {
        ...
        this.pluginsWithUpdates.registerCallback((value, id) => {
            ...
            if (
                settings.store.neverNotifyForPlugins
                    .split(",")
                    .map(t => t.trim().toLowerCase())
                    .includes(value.plugins[value.plugins.length - 1].toLowerCase())
            )
                return;
