...
function Controls() {
    const [isPlaying, shuffle, repeat] = useStateFromStores([TidalStore], () => [
    ...
    const [nextRepeat, repeatClassName] = (() => {
        switch (repeat) {
            case 0:
                return [1, "repeat-off"] as const;
            case 1:
                return [2, "repeat-context"] as const;
            case 2:
                return [0, "repeat-track"] as const;
            default:
    ...
    return (
        <Flex className={cl("button-row")} style={{ gap: 0 }}>
            ...
            <Button
                onClick={() => {
                    Settings.plugins.MusicControls.previousButtonRestartsTrack && TidalStore.position > 3000
                        ? TidalStore.seek(0)
                        : TidalStore.previous();
                }}
            >
...
function TdlSeekBar() {
    ...
    const [storePosition, isPlaying] = useStateFromStores([TidalStore], () => [
...
function Info({ track }: { track: NonNullable<PlayerState["track"]> }) {
    ...
    if (coverExpanded && img) return <div id={cl("album-expanded-wrapper")}>{i}</div>;
            ...
            <div id={cl("titles")}>
                <BaseText
                    ...
                    {...makeLinkProps(
                        `"${track.name}"`,
                        track?.id,
                        track?.url || `https://tidal.com/track/${track?.id}`
                    )}
                >
                    {track?.name}
                </BaseText>
