...
function Controls() {
    const [isPlaying, shuffle, repeat] = useStateFromStores([SpotifyStore], () => [
    ...
    return (
        <Flex className={cl("button-row")} gap="0">
            ...
            <Button
                onClick={() => {
                    settings.store.previousButtonRestartsTrack && SpotifyStore.position > 3000
                        ? SpotifyStore.seek(0)
                        : SpotifyStore.prev();
                }}
            >
...
function SpotifySeekBar() {
    ...
    const [storePosition, isSettingPosition, isPlaying] = useStateFromStores([SpotifyStore], () => [
